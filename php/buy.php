...
if (isset($_SESSION['correo'])) {
    ...
    $queryUserId = "SELECT id, creditos FROM usuarios WHERE correo = '$email'";
    $resultUserId = mysqli_query($conn, $queryUserId);
    if ($rowUserId = mysqli_fetch_assoc($resultUserId)) {
        $userId = $rowUserId['id'];
        $userCredits = $rowUserId['creditos'];
        ...
        if ($userCredits >= $totalPrice) {
            ...
            if ($conn->query($insertOrderQuery) === TRUE) {
                ...
                if ($allQueriesSuccessful) {
                    ...
                    $conn->rollback(); // Rollback transaction
                    echo '<div class="alert alert-danger" role="alert">Error en el proceso de compra.</div>';
                    echo "<br>";
                    echo "<br>";
            ...
            echo '<div class="alert alert-danger" role="alert">No tiene suficientes créditos para completar esta compra.</div>';
            echo "<br>";
            echo "<br>";
            echo '<a href="../html/index.php" class="btn btn-primary center-button" style="padding: 10px 20px; font-size: 16px;">Regresar al inicio</a>';
        }
